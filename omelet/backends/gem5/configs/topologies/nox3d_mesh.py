...
from omelet.nox.hierarchy.fullblown_vertical import build_vertical_inter_chiplet
from omelet.nox.hierarchy.connect_noi_noc_meshmesh import connect_noi_noc

from omelet.nox.builders import *


class nox3d_mesh(SimpleTopology):
    description = "nox3d_mesh"
    ...
    def makeTopology(self, opts, net, IntLink, ExtLink, Router):
        ...
        self.nol_cfg = yaml.safe_load(open(opts.nol_config))
        ...
        for chiplet_i in range(num_chiplets):
            ...
            for _ in range(num_routers):
                noc_routers.append(Router(router_id=router_id,
                                          latency=self.sys_cfg["router_latency"]["chiplets"][role],
                ...
                chiplet_clk_domain = SrcClockDomain(
                    ...
                    voltage = self.sys_cfg["sys_voltage"]))
                ext_links.append(ExtLink(link_id=link_cnt,
                                        ext_node=nodes[total_cpus + router_id],
                                        int_node=noc_routers[router_id],
                                        width=self.sys_cfg["onchip_width"],
                                        clk_domain=chiplet_clk_domain,
                                        latency=self.sys_cfg["ext_latency"]))
        ...
        for i in range(self.noi_cfg["num_routers"]):
            ...
            noi_routers.append(Router(router_id=router_id,
                                      latency=self.sys_cfg["router_latency"]["interposer"],
                                      width=noi_rtr_width,
                                      role=role))
            router_map[role].append(router_id)
            router_id += 1
        ...
        chiplet_grid = {}
        for chiplet_name, chiplet_data in self.plc_cfg["chiplets"].items():
            x, y, z = chiplet_data["logical"]
            key = (x, y)
            if key not in chiplet_grid:
                chiplet_grid[key] = {}
            chiplet_grid[key][z] = chiplet_name

        vertical_pairs = []
        for (x, y), z_map in chiplet_grid.items():
            sorted_layers = sorted(z_map.keys(), reverse=True)
            for i in range(len(sorted_layers) - 1):
                z_top = sorted_layers[i]
                z_bot = sorted_layers[i + 1]
                top_chiplet = z_map[z_top]
                bot_chiplet = z_map[z_bot]
                vertical_pairs.append((top_chiplet, bot_chiplet))

        for pair_i, (top_chiplet, bot_chiplet) in enumerate(vertical_pairs):
            intra_int, num_new_links = build_vertical_inter_chiplet(
                opts, self.link_lib,
                self.noc_cfg[int(top_chiplet.replace("chiplet", ""))],
                self.noc_cfg[int(bot_chiplet.replace("chiplet", ""))],
                self.nol_cfg, self.sys_cfg, self.plc_cfg,
                net, net.routers, IntLink,
                router_map[top_chiplet][0], router_map[bot_chiplet][0],
                link_cnt, top_chiplet, bot_chiplet
            )
            int_links.extend(intra_int)
            link_cnt += num_new_links
